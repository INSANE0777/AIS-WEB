...
export default function ParallaxHero() {
  ...
  useEffect(() => {
    // GSAP animation logic remains unchanged
    ...
    if (titleRef.current) { 
      ...
      titleRef.current.innerHTML = chars .map( (char, i) => `<span class="char inline-block" style="opacity: 0; transform: translate3d(0, 0, 0);">${char === " " ? "&nbsp;" : char}</span>`, ) .join(""); 
    ...
    document.querySelectorAll('.tech-logo').forEach((logo) => { 
      logo.addEventListener('mouseenter', () => { gsap.to(logo, { scale: 1.2, opacity: 1, duration: 0.3, ease: "power2.out" }) }); 
      logo.addEventListener('mouseleave', () => { gsap.to(logo, { scale: 1, opacity: 1, duration: 0.3, ease: "power2.out" }) }); 
    });
  ...
  return (
    ...
    >
      {/* Desktop Logo Layout (Centered) - Made Larger */}
      {!isMobile && (
        <div 
          ref={logoRef}
      ...
      {/* Mobile BIAS Logo Layout (Top Left) */}
      {isMobile && (
      ...
      <div className={`relative z-10 text-center w-full max-w-6xl mx-auto px-4 ${isMobile ? 'pt-20 ml-8' : 'pt-20'}`}>
        
        {/* Cabinet and Bennett logos for mobile - Made Larger */}
        ...
        <div className="mb-6">
          <div className="inline-flex items-center space-x-2 bg-black text-white px-4 py-2 rounded-full text-xs sm:text-sm font-medium animate-fade-in-up group">
            <CustomStar />
