...
import {
  ...
  Megaphone, Globe, Eye, Cpu, MessageSquare, Bot, RefreshCw, PenTool,
} from "lucide-react"
...
const FloatingSvgBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const morphTargets = svgs.map(svgString => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      particle.innerHTML = svgs[i % svgs.length];
      particle.className = "background-svg-particle absolute";
      const path = particle.querySelector('path');
      if (!path) continue;
      
      const size = Math.random() * 150 + 50;
      gsap.set(particle, {
        width: size, height: size, top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%`, opacity: 0,
      });
      container.appendChild(particle);

      gsap.to(particle, { opacity: Math.random() * 0.05 + 0.02, duration: 2, delay: Math.random() * 2 });

      gsap.to(particle, {
        duration: Math.random() * 40 + 30, x: (Math.random() - 0.5) * 400, y: (Math.random() - 0.5) * 400,
        rotation: (Math.random() - 0.5) * 360, ease: "sine.inOut", repeat: -1, yoyo: true, delay: Math.random() * 5,
      });
      
      const shuffledTargets = gsap.utils.shuffle([...morphTargets]);
      const morphTl = gsap.timeline({
          repeat: -1, yoyo: true, delay: Math.random() * 4,
          defaults: { duration: gsap.utils.random(4, 8), ease: "sine.inOut" }
      });
      shuffledTargets.forEach(target => { morphTl.to(path, { morphSVG: target }, "+=1"); });
    }
...
interface MagneticButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> { children: React.ReactNode }
const MagneticButton: React.FC<MagneticButtonProps> = ({ children, ...props }) => <button {...props}>{children}</button>

interface MobileOptimization { isMobile: boolean; reducedMotion: boolean }
const useMobileOptimization = (): MobileOptimization => {
  ...
  useEffect(() => {
    ...
    checkMobile(); checkMotionPreference();
...
const NON_TECH_DEPARTMENTS: { [key: string]: DepartmentInfo } = { 
...
const TIMELINE_ITEMS = [{ icon: Send, title: "Fill the Form", subtitle: "Applications open from 6th November to 15th November", description: "Applications: 128+", status: "ongoing", date: "6th Nov - 15th Nov" }, { icon: Clock, title: "Form Shortlisting", subtitle: "Form review period where you will be judged by the form responses", description: "AI Generated Form Responses will be rejected.", status: "upcoming", date: "Date to be announced" }, { icon: MessageCircle, title: "One-on-One Interview", subtitle: "Candidate is interviewed on technicalities and soft skills", description: "Interview Guidelines and Preparation", status: "upcoming", date: "Date to be announced" }]
          ...
          <div className="inline-flex items-center space-x-2 bg-black text-white px-6 py-3 rounded-full text-sm font-medium mb-8">
            ...
            >
              <g clipPath="url(#clip0_119_300)">
                <path d="M99.6778 105.287C99.6778 -81.6924 145.108 21.3021 98.3534 102.278C145.098 21.3021 257.091 9.12898 95.091 102.638C257.052 9.14845 190.528 99.9892 97.0387 99.9892C190.528 99.9892 257.062 190.859 95.091 97.3404C257.052 190.83 145.108 178.686 98.3534 97.7007C145.098 178.686 99.6778 281.759 99.6778 94.7012C99.6778 281.681 54.2379 178.686 100.993 97.7007C54.2477 178.686 -57.7451 190.859 104.255 97.3404C-57.7062 190.83 8.81757 99.9892 102.307 99.9892C8.81757 99.9892 -57.7159 9.12898 104.255 102.638C-57.7062 9.14845 54.2379 21.3021 100.993 102.278C54.2379 21.3021 99.6778 -81.7411 99.6778 105.287Z" fill="white" />
              </g>
              <defs>
        ...
        <section className="form-section my-12">
          ...
          {activeGuideline === "tech" && (
            <div className="bg-white p-8 rounded-2xl border border-black/10">
              <div className="flex items-center space-x-3 mb-6"><AlertTriangle className="text-red-500" size={24} /><h3 className="text-2xl font-bold text-black">For Technical Positions</h3></div>
              <div className="space-y-6 text-black/70">
                <div><p>AI Generated Form Responses will be rejected in review.</p></div>
                <div><h4 className="font-bold text-black mb-2">Create a Kaggle account.</h4><p>Kaggle is a free, online community and platform for AI competitions, learning, and collaboration. It is used by recruiters worldwide.</p></div>
                <div><h4 className="font-bold text-black mb-2">Mandatory Courses</h4><p className="mb-2">Complete the following mandatory courses on Kaggle and obtain the certificates. These courses are covered in the YouTube course by Abhishek Thakur's Kaggle Tutorial for Beginners (World's first 4x Kaggle Grandmaster)</p><ul className="list-disc list-inside space-y-1"><li><a href="https://www.kaggle.com/learn/python" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Python Course</a> (Mandatory)</li><li><a href="https://www.kaggle.com/learn/intro-to-machine-learning" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Intro to Machine Learning</a> (Mandatory)</li></ul></div>
                <div><h4 className="font-bold text-black mb-2">Optional Courses</h4><ul className="list-disc list-inside space-y-1"><li><a href="https://www.kaggle.com/learn/intro-to-deep-learning" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Intro to Deep Learning</a> (Optional)</li><li><a href="https://www.kaggle.com/learn/intermediate-machine-learning" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Intermediate Machine Learning</a> (Optional)</li></ul></div>
                <div><p>Post Certificate on LinkedIn tagging <a href="https://www.linkedin.com/company/ais-bennett/posts/?feedView=all" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">@Artificial Intelligence Society Bennett University</a>, mentioning the completion of these (2) courses, and include the certificate links in your post. (Optional, will be considered)</p></div>
                <div><h4 className="font-bold text-black mb-2">For the interview, we will:</h4><ul className="list-disc list-inside space-y-1"><li>Verify your Kaggle account and check the completion of these (2) courses along with their certificates.</li><li>You will be asked questions regarding the Kaggle courses.</li><li>Ask you for sure: Why do you want to join the CV / RL / GenAI / NLP department? Share a problem you want to solve with AI in this domain and your approach.</li></ul></div>
                <div><p>Bring your laptop to the interview. Ensure it's fully charged and ready to showcase your work.</p></div>
              </div>
            </div>
          )}
          {activeGuideline === "community" && (<div className="bg-white p-8 rounded-2xl border border-black/10"><div className="flex items-center space-x-3 mb-6"><AlertTriangle className="text-blue-500" size={24} /><h3 className="text-2xl font-bold text-black">For Non-Technical Positions</h3></div><div className="space-y-6 text-black/70"><div><h4 className="font-bold text-black mb-2">For Creative and Multimedia Teams:</h4><ul className="list-disc list-inside space-y-1"><li>Submit your portfolio (designs, videos, writing samples) in the form and be prepared to present them during the interview.</li><li>Skills required may include: Figma, Canva, Adobe Suite, Video Editing Software, strong writing abilities, etc.</li><li>Note: Creative Team consists of Design, Social Media, Editorial, and PR/Outreach.</li></ul></div><div><h4 className="font-bold text-black mb-2">For Management and PR/Outreach Teams:</h4><ul className="list-disc list-inside space-y-1"><li>Your behavior, personality, and speaking skills will be assessed through situational questions and potentially an on-the-spot task during the interview.</li><li>Be prepared to discuss past experiences in leadership or public relations.</li></ul></div><p>Bring your laptop to the interview to showcase any relevant work.</p></div></div>)}
          {activeGuideline === "general" && (<div className="bg-white p-8 rounded-2xl border border-black/10"><div className="flex items-center space-x-3 mb-6"><AlertTriangle className="text-green-500" size={24} /><h3 className="text-2xl font-bold text-black">General Guidelines for All</h3></div><div className="space-y-4 text-black/70"><ul className="list-disc list-inside space-y-2"><li><strong>Be on time.</strong> Punctuality is crucial.</li><li><strong>Dress appropriately.</strong> Professional attire is expected.</li><li><strong>Bring your laptop</strong> and any necessary accessories.</li><li><strong>Be prepared to discuss</strong> your work, skills, and experience in detail.</li><li><strong>Showcase relevant projects.</strong> Highlight the work that best represents your abilities.</li><li><strong>Ask questions.</strong> Engage with the interviewers by asking insightful questions.</li></ul></div></div>)}
        </section>
