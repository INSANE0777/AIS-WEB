...
import { Users, Sparkles, Award, Brain, Heart, Zap, Star, Rocket, ArrowRight, Send, LoaderIcon, CheckCircle, XCircle } from "lucide-react";
...
const ALL_DEPARTMENTS: AllDepartments = {
  Tech: ["Natural Language Processing (NLP)", "Generative AI (GenAI)", "Reinforcement Learning (RL)", "Computer Vision (CV)"],
  NonTech: Object.keys(NON_TECH_DEPARTMENTS),
};

const ADDITIONAL_SKILLS: string[] = [ 
  "Researching", "Content Writing", "Designing (Canva/Figma/Photoshop etc.)", "Public Speaking", 
  "Outreach and Sponsorship", "Social Media Management / Marketing", "Photography/Videography", 
  "Managing Events", "3D softwares (Blender, Maya, etc)", "Video editing (Capcut/Kinemaster/After Effects, Da Vinci etc)"
];
...
const getFriendlyErrorMessage = (error: any): string => {
  ...
  if (message.toLowerCase().includes("permission")) return "Permission Denied: You may not have the required permissions to submit this form. Please contact support.";
  if (message.toLowerCase().includes("database or collection is not configured")) return "Configuration Error: The form is not set up correctly. Please contact the administrator.";
  ...
  const SuccessMessage = () => (
    <div className="text-center bg-white p-8 sm:p-12 rounded-2xl border border-green-300 shadow-lg">
      ...
      <p className="text-black/70 text-lg mb-8 max-w-lg mx-auto">
        Thank you for your interest in joining the AI Society. We have received your application and will review it shortly.
      </p>
      <MagneticButton
        ...
        <div className="form-section mb-12">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {Object.entries(NON_TECH_DEPARTMENTS).map(([dept, info]) => {
              ...
              return (
                <div key={dept} className="bg-white p-6 rounded-xl border border-black/10 hover:border-black hover:shadow-lg transition-all group">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {info.skills.map((skill) => (
                      <span key={skill} className="px-2 py-1 bg-black/5 text-black text-xs rounded-full">{skill}</span>
                    ))}
                  </div>
                </div>
              );
        ...
        <div className="form-section bg-white p-8 rounded-2xl border border-black/10 hover:border-black/20 transition-all">
          ...
          {submissionStatus === 'success' ? (
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">Full Name <span className="text-red-500">*</span></label>
                  <input 
                    type="text" 
                    {...register("fullName")} 
                    required 
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">Phone Number <span className="text-red-500">*</span></label>
                  <input 
                    type="tel" 
                    {...register("phone")} 
                    required 
                ...
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">Degree Program <span className="text-red-500">*</span></label>
                  <input 
                    type="text" 
                    {...register("degree")} 
                    required 
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">Current Year <span className="text-red-500">*</span></label>
                  <select 
                    {...register("year")} 
                    required 
                    className="w-full px-4 py-3 border-2 border-black/20 rounded-full focus:outline-none focus:border-black transition-colors hover:border-black/40"
                ...
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">Hostel Accommodation <span className="text-red-500">*</span></label>
                  <select 
                    {...register("hostel")} 
                    required 
                    className="w-full px-4 py-3 border-2 border-black/20 rounded-full focus:outline-none focus:border-black transition-colors hover:border-black/40"
              ...
              {domain === "Tech" && (
                <div className="grid md:grid-cols-2 gap-6 p-4 bg-blue-50 rounded-2xl border border-blue-200">
                  <div>
                    <label className="block text-sm font-semibold text-black mb-2">GitHub Profile <span className="text-red-500">*</span></label>
                    <input 
                      type="text" 
                      {...register("githubLink")} 
                      required 
              ...
              <div>
                ...
                >
                  ...
                  {domain && ALL_DEPARTMENTS[domain === "Tech" ? "Tech" : "NonTech"].map((dept) => (
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">Elaborate on Your Choices <span className="text-red-500">*</span></label>
                  <textarea 
                ...
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">Hobbies <span className="text-red-500">*</span></label>
                  <textarea 
                    {...register("hobbies")} 
                    required 
                    rows={4} 
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">Fictional Character <span className="text-red-500">*</span></label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">Why Join Us? <span className="text-red-500">*</span></label>
                  <textarea 
                    {...register("whyJoinUs")} 
                    required 
                    rows={4} 
